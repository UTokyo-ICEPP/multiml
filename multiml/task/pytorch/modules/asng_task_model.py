...
class ASNGModel(ConnectionModel, Module):
    def __init__(self, task_ids, lam, delta_init_factor, alpha=1.5, range_restriction=True, *args, **kwargs):
        """
        Args:
        ...
        for task, task_id in zip(self._models, task_ids):
            ...
            key_to_idx[task_id] = {}
            hps_params = task.get_hps_parameters() # this should like {'choice_block_model_name':N} or {'hps0':N0, 'hps1':N1,,,}
    ...
    def convert_cat_to_hps_choice(self, c_cat):
        choice = { task_id : { key : None for key in keys} for task_id, keys in self.task_id_keys.items() }
