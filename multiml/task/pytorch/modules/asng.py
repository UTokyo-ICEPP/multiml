...
class AdaptiveSNG:
    ...
    """
    def __init__(self, categories = None, integers = None, alpha=1.5, delta_init=1., lam=2, delta_max=np.inf, init_theta=None):
        """ AdaptiveSNG
        """
    ...
    def sampling_cat(self ):
        ...
        rand = np.random.rand( self.lam, self.cat_d, 1)  # range of random number is [0, 1)
    ...
    def update_theta(self, c_cat, c_int, losses, range_restriction=True):
        ...
        if self.theta_cat is not None : 
            ...
            for i in range(self.cat_d):
                ...
                theta_min = 1. / (self.cat_d_valid * (ci - 1)) if range_restriction and ci > 1 else 1e-04
